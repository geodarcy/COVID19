...
#   curbsideDF = df.loc[(df['UrbanArea'] == city) & (df['area_sqkm'] <= 250)].nlargest(50, 'gt200_curbside_similarity')
...
#   outDF[['UrbanArea', 'self_public_space_similarity', 'self_mobility_similarity', 'gt200_curbside_similarity', 'full_text']].to_csv(os.path.join(path, 'CodedTopics', 'CityTweets', city + 'TopTweets.csv'))
...
analysis_columns = ['self_public_space_similarity', 'self_mobility_similarity', 'gt200_curbside_similarity']
...
city_dict = dict(zip(cities, final_cities))
out_fields = ['created_at', 'full_text', 'gt200_curbside_similarity', 'area_sqkm', 'UrbanArea', 'geometry']
for city in cities:
  tempDF = geoDF.loc[geoDF['UrbanArea'] == city_dict[city], out_fields]
  tempDF['geometry'] = tempDF['geometry'].centroid
  tempDF.columns = ['created_at', 'full_text', 'curbside', 'area_sqkm', 'UrbanArea', 'geometry']
...
for city in cities:
  ...
  y = grouped['gt200_curbside_similarity'].mean()
  s = grouped['gt200_curbside_similarity'].count() / 2
  z = np.polyfit(x, y, 1)
  p = np.poly1d(z)
...
list(df.loc[(df['TweetDay'] == max_day) & (df['UrbanArea'] == city_dict[city])].sort_values('gt200_curbside_similarity', ascending=False)['full_text'])
df.loc[(df['TweetDay'] == max_day) & (df['UrbanArea'] == city_dict[city]), 'gt200_curbside_similarity'].describe()
list(df.loc[df['UrbanArea'] == city_dict[city]].sort_values('gt200_curbside_similarity', ascending=False)['full_text'])[:10]
...
out_fields = ['created_at', 'Month', 'full_text', 'self_public_space_similarity', 'self_mobility_similarity', 'gt200_curbside_similarity', 'area_sqkm', 'UrbanArea', 'geometry']
...
# export top tweets
for city in cities:
  df.loc[df['UrbanArea'] == city_dict[city], ['TweetDay', 'UrbanArea', 'place.name', 'gt200_curbside_similarity', 'full_text']].sort_values('gt200_curbside_similarity', ascending=False).head(1000).to_csv(os.path.join(path, 'Paper', 'SampleTweets', city + '_curbside_tweets_Reduced.csv'))
# for city in cities:
...
df[['TweetDay', 'UrbanArea', 'place.name', 'gt200_curbside_similarity', 'full_text']].sort_values('gt200_curbside_similarity', ascending=False).head(250).to_csv(os.path.join(path, 'Paper', 'SampleTweets', 'all_curbside_tweets_Reduced.csv'))
# one output for 8 selected cities
df.loc[df['UrbanArea'].isin([final_cities[i] for i in [1, 3, 4, 7]]), ['TweetDay', 'UrbanArea', 'place.name', 'gt200_curbside_similarity', 'full_text']].sort_values('gt200_curbside_similarity', ascending=False).head(1000).to_csv(os.path.join(path, 'Paper', 'SampleTweets', 'all_curbside_tweets_FinalCities_Reduced.csv'))

# plot similarity score distribution (Figure 3)
fig, ax1 = plt.subplots(nrows=1, sharex=True, sharey=True, figsize=(6,2))
ax1.hist(df['gt200_curbside_similarity'], bins=50)
ymin, ymax = ax1.get_ylim()
ax1.set_xlim(-0.25, 1)
ax1.vlines(df['gt200_curbside_similarity'].mean(), ymin, ymax, label='mean')
# ax1.text(0.1, 17500, 'curbside')
ax1.set_ylabel('Number of tweets')
ax1.set_xlabel('Similarity Score')
ax1.legend()
fig.savefig(os.path.join(path, 'Images', 'TopicHistogramCurbside.png'), dpi=300, bbox_inches='tight', pad_inches=0.05)
